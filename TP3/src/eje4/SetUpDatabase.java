...
public class SetUpDatabase {
	...
	public void setUp() {
		jdbi.useTransaction(handle -> {
			...
			handle.createUpdate(INSERT).bind(0, "Jose").bind(1, "Laurenti").execute();

			handle.createUpdate(INSERT).bind(0, "Esteban").bind(1, "Otermon").execute();

			handle.createUpdate(INSERT).bind(0, "Vladimir").bind(1, "Varkov").execute();
		});
	}
}